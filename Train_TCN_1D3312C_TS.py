...
def train_model(result_path, 
    ...
    train_dt =  HazardNet_DL_Helpers.train_dataloader(X=X_train, y=lbe_train, TW_data_num=36, batch_size=batch_size_train, replacement=replacement, num_workers=16, sampler_active=False,print_txt_file_name=print_txt_file_name, Conv='1D')
    val_dt =  HazardNet_DL_Helpers.val_dataloader(X=X_val ,y=lbe_val, TW_data_num=36, batch_size=batch_size_val, num_workers=16, Conv='1D')
    ...
    model = HazardNet_Models.Net_1D3312C(dropout=dropout).double() 
...
root_path = './TRAIN_RESULTS/Inlet/1D3312C_predictor_may_1D_Inlet_paper/'
